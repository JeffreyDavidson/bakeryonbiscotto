    ...
    <x-admin.card title="Customer Notes" :subtitle="(string) $customerNotes->count()">
        <div style="padding: 1rem 1.25rem;">
            ...
            @foreach($customerNotes->where('is_important', true) as $cn)
                <div style="padding:0.75rem 1rem;margin-bottom:0.5rem;background:#fffbeb;border:1px solid #f59e0b;border-radius:0.5rem;">
                    <div style="display:flex;align-items:start;gap:0.5rem;">
                        <span style="color:var(--status-warning);font-size:1rem;line-height:1;">&#9888;</span>
                        <div style="flex:1;">
                            <div style="font-size:0.85rem;color:var(--status-warning-dark);font-weight:600;line-height:1.4;">{{ $cn->note }}</div>
            ...
            @foreach($customerNotes->where('is_important', false) as $cn)
                <div style="padding:0.75rem 1rem;margin-bottom:0.5rem;background:var(--brand-50);border:1px solid var(--brand-200);border-radius:0.5rem;">
                    <div style="font-size:0.85rem;color:var(--brand-900);line-height:1.4;">{{ $cn->note }}</div>
                    <div style="font-size:0.7rem;color:var(--brand-500);margin-top:0.25rem;">{{ $cn->created_by ?? 'Unknown' }} · {{ $cn->created_at->format('M j, Y g:i A') }}</div>
                </div>
            @endforeach

            @if($customerNotes->isEmpty())
                <div style="font-size:0.85rem;color:var(--brand-400);font-style:italic;">No notes yet.</div>
            @endif

            {{-- Add Note Form --}}
            <div x-data="{ note: '', isImportant: false, saving: false }" style="margin-top:0.75rem;padding-top:0.75rem;border-top:1px solid var(--brand-200);">
                <textarea x-model="note" placeholder="Add a note about this customer..." rows="2" style="width:100%;border:1px solid var(--brand-200);border-radius:0.5rem;padding:0.5rem 0.75rem;font-size:0.85rem;resize:vertical;background:#fff;color:var(--brand-900);font-family:inherit;"></textarea>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-top:0.5rem;">
                    <label style="display:flex;align-items:center;gap:0.375rem;font-size:0.8rem;color:var(--status-warning-dark);cursor:pointer;">
                        <input type="checkbox" x-model="isImportant" style="accent-color:var(--status-warning);">
                    ...
                    <button
                        ...
                        style="padding:0.375rem 1rem;background:var(--brand-600);color:white;border:none;border-radius:0.375rem;font-size:0.8rem;font-weight:600;cursor:pointer;opacity:1;transition:opacity 0.15s;"
    ...
    <x-admin.card title="Favorite Products" :subtitle="(string) $favoriteProducts->count()">
        <div style="padding: 1rem 1.25rem; display: flex; flex-wrap: wrap; gap: 0.5rem;">
            @foreach($favoriteProducts as $fp)
                <span style="display:inline-flex;align-items:center;gap:0.375rem;padding:0.375rem 0.75rem;background:#fef2f2;border:1px solid #fecaca;border-radius:2rem;font-size:0.8rem;color:var(--status-danger-dark);">
                    ❤️ {{ $fp->name }}
                </span>
            @endforeach
        </div>
